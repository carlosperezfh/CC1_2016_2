...
Polygon::Polygon(const PointArray &pA)
    : ptArr{pA} {   //pa se mete en ptArr
    ++numPol;
}

Polygon::Polygon(const Polygon &pol)
    : ptArr{pol.ptArr} {   //guarda pol.Ptarr en ptArr de este poligono
    ++numPol;
}
...
Rectangle::Rectangle(const Point &p1, const Point &p2)  //esta hablando del public:     Polygon(const Point points[], const int &len);
    : Polygon{updateConstructorPoints(p1, Point{p1.getX(), p2.getY()},
    p2, Point{p2.getX(), p1.getY()}), 4} {}

Rectangle::Rectangle(const int &p1x, const int &p1y, const int &p2x,    //recibimos 4 por separado y arriba 2
                     const int &p2y)
